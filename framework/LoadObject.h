...
void IndexVBO(
	...
	std::vector<Vector3> & in_tangents,
	std::vector<Vector3> & in_bitangents,
	...
	)
{
	...
	for (unsigned int i = 0; i < in_vertices.size(); ++i)
	{
		...
		else
		{
			...
			v.tangent.Set(in_tangents[i].x, in_tangents[i].y, in_tangents[i].z);
			v.bitangent.Set(in_bitangents[i].x, in_bitangents[i].y, in_bitangents[i].z);
	...
	)
{
	for (unsigned int i = 0; i < vertices.size(); i += 3) {
		...
		// Shortcuts for UVs
		TexCoord & uv0 = uvs[i + 0];
		TexCoord & uv1 = uvs[i + 1];
		TexCoord & uv2 = uvs[i + 2];
		...
		Vector3 tangent = (deltaPos1 * deltaUV2.v - deltaPos2 * deltaUV1.v)*r;
		Vector3 bitangent = (deltaPos2 * deltaUV1.u - deltaPos1 * deltaUV2.u)*r;
